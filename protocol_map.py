...
| `run_lan_dashboard` | `networked_sensors/run_lan_dashboard.sh` | exists | environment-selected Yún URL, DXMR90 host, ESP32 source/URL, bind host/port, plus optional dashboard CLI arguments | one-command production-LAN dashboard process | defaults to network Yún at `http://arduino.local:8080`, real DXMR90 at `192.168.0.1`, ESP32 off, and `0.0.0.0:8000` |
...
| `limit_switch_palas` | `networked_sensors/limit_switch_palas.ino` | T6 raw-UART firmware compiles at 72% flash/54% RAM, is uploaded with the 251.96850394 pulses/mm calibration, and emits stopped USB and LAN status | D4, D5, D6/D8, USB or Linux-relayed `V1 S25..2520`/`V1 D`/`V1 M`, armed `V1 H`/relative `V1 G`, Web Position `V1 X`, and mode-independent latched `V1 E1` with D4-off `V1 E0` | Local Velocity plus relative Web Position, D8 seek, fixed acceleration, directional limits, immediate abort, software E-STOP, non-blocking Serial1 status/acks, and exclusive USB/network mutation ownership; 0.00396875 mm/PUL assumes photographed DM542T SW5-SW8 all ON; no absolute-position safety gate or safety-rated energy isolation | compile/upload as `arduino:avr:yun`; because the repository file is not in an Arduino-named sketch directory, stage it in a matching temporary sketch directory first |
| `yun_stepper_bridge` | `networked_sensors/yun_stepper_bridge.py` | Python 2/3 loopback plus physical health, stopped status, rejection, and boot-start pass; motion tests pending | compact ATmega status plus exact validated `V1` command lines on `/dev/ttyATH0` | trusted-LAN `GET /v1/status`, `GET /v1/health`, and `POST /v1/command` on port 8080 | init wrapper temporarily disables LEDEYun `askconsole` while running and restores it on stop; provisioner installs and enables it |
...
| Yún stepper USB | `UsbStepperSource` | adapter/firmware uploaded; stopped latch/reset and calibrated `csps:378` live smokes passed; moving E-STOP checks pending | state changes, 10 Hz moving status, 1 Hz idle heartbeat; mode/speed/mapping, D8 seek/move/Stop, and software E-STOP/reset | dual control modes, D4/D5 authority, D6/D8 directional stops, 251.96850394 pulses/mm, fixed acceleration, command/state/reason/capabilities, and explicit E-STOP latch; absolute position suppressed | same stepper health fields plus `stepper_transport_error`; software stop is not safety-rated |
...
| stepper unit tests | `python3 -m unittest -v networked_sensors.test_stepper_control` | D5-selected travel, validation, stops, limits, dashboard runtime, latched software E-STOP/reset, USB/network transport behavior, and stable merged shape | No; 33 tests passed |
...
| Yún T6 compile/upload | `arduino-cli compile/upload --fqbn arduino:avr:yun ...` | dual USB/Serial1 transport, ownership, calibrated motion, limits, and E-STOP use 20,782 bytes/72% flash and 1,399 bytes/54% RAM; post-upload stopped USB heartbeat reports `csps:378` at the D6 limit | Yún USB plus trusted WLAN; compile/upload, calibrated stopped status, and stopped LAN boot pass; dimensional and high-rate motion qualification pending |
